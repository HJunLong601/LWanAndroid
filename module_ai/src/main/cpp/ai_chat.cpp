...
Java_com_arm_aichat_internal_InferenceEngineImpl_init(JNIEnv
*env, jobject /*unused*/,
...
env->
ReleaseStringUTFChars(nativeLibDir, path_to_backend
);
...
JNIEXPORT jint
JNICALL
        Java_com_arm_aichat_internal_InferenceEngineImpl_load(JNIEnv * env, jobject, jstring
jmodel_path) {
...
env->
ReleaseStringUTFChars(jmodel_path, model_path
);
if (!model) {
return 1;
}
...
JNIEXPORT jstring
JNICALL
        Java_com_arm_aichat_internal_InferenceEngineImpl_benchModel(JNIEnv * env,
                                                                    jobject /*unused*/, jint
pp,
jint tg,
        jint
...
char model_desc[128];
llama_model_desc(g_model, model_desc,
sizeof(model_desc));
...
static void shift_context() {
    ...
    llama_memory_seq_add(llama_get_memory(g_context), 0, system_prompt_position + n_discard,
...
env->
ReleaseStringUTFChars(jsystem_prompt, system_prompt
);
...
env->
ReleaseStringUTFChars(juser_prompt, user_prompt
);

// Decode formatted user prompts
auto user_tokens = common_tokenize(g_context, formatted_user_prompt, has_chat_template,
                                   has_chat_template);
for (
auto id
: user_tokens) {
...
user_tokens.
resize(max_batch_size);
LOGw("%s: User prompt too long! Skipped %d tokens!", __func__, skipped_tokens);
}

// Decode user tokens in batches
if (
decode_tokens_in_batches(g_context, g_batch, user_tokens, current_position,
true)) {
LOGe("%s: llama_decode() failed!", __func__);
return 2;
}
...
Java_com_arm_aichat_internal_InferenceEngineImpl_generateNextToken(
    ...
    if (is_valid_utf8(cached_token_chars.c_str())) {
        ...
        LOGv("id: %d,\tcached: `%s`,\tnew: `%s`", new_token_id, cached_token_chars.c_str(),
...
extern "C"
JNIEXPORT void JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_unload(JNIEnv
* /*unused*/, jobject /*unused*/) {
// Reset long-term & short-term states
reset_long_term_states();
...
extern "C"
JNIEXPORT void JNICALL
Java_com_arm_aichat_internal_InferenceEngineImpl_shutdown(JNIEnv
*, jobject /*unused*/) {
llama_backend_free();
